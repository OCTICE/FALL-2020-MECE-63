% TAYLOR'S METHOD OF ORDER TWO:
 ...
 %syms('F','DF','OK','A','B','ALPHA','N','FLAG','NAME','OUP','H');
 %syms('T','W','I','x','s');
 TRUE = 1;
 FALSE = 0;
 fprintf(1,'This is Tayors Method of order two.\n');
 fprintf(1,'Input the function F(t,y) in terms of t and y\n');
 fprintf(1,'For example: y-t^2+1\n');
 s = input(' ','s');
 F = inline(s,'t','y');
 fprintf(1,'Input the function dF(t,y)/dt in terms of t and y\n');
 fprintf(1,'For example: y-t^2+1-2*t\n');
 s = input(' ','s');
 DF = inline(s,'t','y');
 ...
 fprintf(OUP, 'Taylor Method of Order 2\n\n');
 ...
 if OKS==TRUE
   ...
   W = ALPHA;
   fprintf(OUP, '    t        w               y-w\n\n');
   fprintf(OUP, '%6.4f %15.8e %15.8e\n', T, W, W-W);
 else
   W = ALPHA;
   fprintf(OUP, '    t        w  \n\n');
   fprintf(OUP, '%6.4f %15.8e \n', T, W);
 end;
...
% STEP 4
 ...
 for i=1:201;
  ...
  if  OKS==TRUE
    plot(xx,AP,'*-'); hold on;
    plot(xxp,yyp,'r-');
    legend('Taylor Method of Order 2','Exact solution', 'Location','NorthWest');
  else
    plot(xx,AP,'*-'); hold on;
    legend('Taylor Method of Order 2','Location','NorthWest');
  end;
